...
## AI 어시스턴트 기본 행동 수칙 (Core Behavior)

- **명시적 지시 전 코드 작성 및 수정 절대 금지**: 사용자가 명시적으로 "코드를 작성해", "수정해"라고 지시하기 전까지는 **절대로 코드를 먼저 출력하거나 임의로 수정안을 제안하지 않습니다.** 아키텍처나 구조에 대한 논의가 진행 중일 때는 개념과 방향성에 대해서만 설명하며, 코드 작성은 반드시 사용자의 명확한 허락과 요청이 있을 때만 수행합니다.
...
| **엔티티**   | `src/models/entities/bronze_news.py` | `BronzeNewsModel` (파이프라인 공통 데이터 객체)             |
...
| **공급자**   | `src/providers/bigquery.py`          | BigQuery 클라이언트 및 세마포어 통합 관리                   |
...
- **이벤트 루프 안전성 확보 (Event Loop Safety)**: `asyncio.Semaphore`와 같은 비동기 락 객체들은 모듈 레벨에서 즉시 인스턴스화하지 않고, FastAPI `lifespan` 컨텍스트 내에서 초기화하여 실행 중인 이벤트 루프와의 충돌을 방지하고 `app.state`를 통해 전역 공유합니다.
...
3. `src/worker/services/ingest.py`의 `get_ingest_service()` 내 `source_plugins` 리스트에 `CollectPlugin(source=NewSource(semaphore=request.app.state.source_semaphore))`를 추가합니다.

## 알려진 동작 특성 (Known Behaviors)

- **`BigQueryProvider` 자격증명 없이 앱 시작 가능**: `src/providers/bigquery.py`의 `BigQueryProvider`는 GCP 자격증명(`K_SERVICE` 환경의 서비스 계정, 또는 로컬의 `GOOGLE_APPLICATION_CREDENTIALS` / `gcloud auth application-default login`)이 없으면 클라이언트를 `None`으로 유지합니다. 앱 자체는 정상 시작되지만, 파이프라인 실행 시 DB 단계에서 `RuntimeError`가 발생하여 `status="failed"`로 처리됩니다.
...
- **JSON Load Job의 `loaded_at` 주입 처리**: 데이터의 모든 변경 이력을 보존하기 위해(Append-Only) `load_table_from_json()` Load Job을 사용합니다. BigQuery Load API가 스키마의 기본값(`CURRENT_TIMESTAMP()`)을 무시하는 특성을 우회하기 위해, Pydantic 모델에는 `loaded_at`을 제외하고 `StoreBase.execute_load_json()`에서 적재 직전에 현재 UTC 시간을 직접 주입합니다.
