...
| **플러그인** | `src/worker/plugins/bigquery.py`     | DB 통신 전담: `lookup()` + `load()` (현재 Mock)             |
...
- **`BigQueryPlugin` Mock**: 현재 `bigquery.py`는 실제 BigQuery 연결 없이 `asyncio.sleep`으로 지연을 시뮬레이션합니다. 실제 구현으로 교체할 때는 `lookup()` / `load()` 시그니처를 유지하고 내부만 교체합니다.
