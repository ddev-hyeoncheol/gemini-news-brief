...
- **`BigQueryPlugin` 자동 Mock 전환**: `get_ingest_service()`에서 `bigquery.Client()` 초기화 시 GCP 자격증명이 없으면 `DefaultCredentialsError`를 잡아 `client=None`으로 설정합니다. `BigQueryPlugin`은 `self.client is None`이면 실제 쿼리 없이 Mock 결과를 반환하므로, 로컬 환경에서도 GCP 자격증명 설정 없이 전체 파이프라인 흐름을 테스트할 수 있습니다.
...
- **`load()` MERGE의 `loaded_at` 처리**: MERGE의 `WHEN NOT MATCHED INSERT`에 `loaded_at`을 포함하지 않아 BigQuery 스키마의 `CURRENT_TIMESTAMP()` 기본값을 활용합니다. `WHEN MATCHED UPDATE`에도 `loaded_at`을 포함하지 않아 최초 적재 시각을 보존합니다.
...
- **`_parse_article` 예외 전파**: `SourceBase._parse_article()`은 newspaper3k 오류 또는 빈 본문 시 예외를 상위로 전파합니다. `CollectPlugin.enrich()`의 `asyncio.gather(..., return_exceptions=True)`가 이를 포착하여 `failed_count`에 정확히 반영하고, `enriched_count`는 실제로 본문 추출에 성공한 기사 수만 집계합니다.
