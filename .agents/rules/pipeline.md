...
- Bronze lookup은 BigQuery full scan을 피하기 위해 최근 window로 제한할 수 있습니다. window 밖 중복은 비용 절감을 위한 허용 trade-off입니다.
...
- `news`는 Enrich에서 `status_code == 200`과 content 존재로 `status == "success"`가 된 bronze record만 `silver.news`로 정규화합니다.
