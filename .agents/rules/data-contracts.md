...
- `target`은 enum 또는 registry로 제한하고 Terraform schema에 반영하지 않습니다.
- `status`, `failed_phase`, phase status는 `Literal` 또는 enum으로 제한합니다.
- count 필드는 pipeline 규칙의 실제 의미와 description을 일치시킵니다.
- DTO-only 필드인 count, status, failed_phase, error_message는 BigQuery Entity 필드일 때만 Terraform에 반영합니다.
...
- **[CRITICAL]** BigQuery column 삭제, rename, REQUIRED 전환처럼 기존 데이터 손실이나 적재 실패 위험이 있는 schema 변경은 명시적으로 확인합니다.
- Terraform `loaded_at` column은 Entity에 두지 않습니다.
- BigQuery JSON Load Job은 `defaultValueExpression`을 평가하지 않으므로 `StoreBase.execute_load_json()`에서 `loaded_at`을 주입합니다.
